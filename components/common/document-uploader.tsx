...
const OnUploadDisplay = () => {
    ...
    const timer = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          clearInterval(timer);
          setTimeout(
            () => updateDocumentUploadStatus(DOCUMENT_UPLOAD_STATUS.SUCCESS),
            500
          );
          return 100;
        }
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      transition={{ duration: 0.3, ease: 'easeOut' }}
      className="w-full overflow-hidden border-dashed border-primary border-[1.5px] rounded-lg px-6 py-8 max-w-[500px] gap-10 flex flex-col"
    >
      <div className="flex flex-col items-center justify-center gap-5 py-4">
        <img src="/pdf.svg" alt="" className="w-[2.75rem] object-contain" />

        <div className="w-full flex flex-col gap-4">
          <div className="flex flex-col items-center gap-1 w-full">
            <p className="text-lg text-[#98A2B3] font-semibold">{progress}%</p>
            <Progress value={progress} className="w-4/5 h-1.5" />
          </div>

          <div className="flex flex-col items-center w-full">
            <p className="text-base font-semibold">Uploading Document{dots}</p>
            <p className="text-sm text-[#98A2B3] ">{document?.name}</p>
          </div>
        </div>
...
const OnSuccessDisplay = () => {
  ...
  useEffect(() => {
    setTimeout(
      () => updateDocumentUploadStatus(DOCUMENT_UPLOAD_STATUS.LOADED_IN_EDITOR),
      1500
    );
  }, [updateDocumentUploadStatus]);
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      transition={{ duration: 0.3, ease: 'easeOut' }}
      className="w-full overflow-hidden border-dashed border-[#5FC381] border-[1.5px] rounded-lg px-6 py-8 max-w-[500px] gap-10 flex flex-col"
    >
      <div className="flex flex-col items-center justify-center gap-5 py-4">
        <img
        ...
        <div className="w-full flex flex-col gap-4">
          <div className="flex flex-col items-center w-full">
            ...
            <p className="text-sm text-[#98A2B3] ">Name of document</p>
          </div>
        </div>

        <div className="text-[#98A2B3] flex items-center justify-center gap-2">
          <ClipLoader size={'0.865rem'} color="currentColor" />
          <p className="text-base text-[#98A2B3] font-semibold">
            Opening document..
          </p>
        </div>
...
const OnErrorDisplay = () => {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      transition={{ duration: 0.3, ease: 'easeOut' }}
      className="w-full overflow-hidden border-dashed border-[#E26E6A] border-[1.5px] rounded-lg px-6 py-8 max-w-[500px] gap-10 flex flex-col"
    >
      <div className="flex flex-col items-center justify-center gap-5 py-4">
        <img
        ...
        <div className="w-full flex flex-col gap-4">
          <div className="flex flex-col items-center w-full">
            <p className="text-base font-semibold">Failed to Upload</p>
        ...
        <button className="text-[#F56630] flex items-center justify-center gap-2 cursor-pointer">
          <RefreshCcw size={'1.25rem'} color="currentColor" />
