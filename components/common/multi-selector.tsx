...
export default function MultiSelector({
  ...
  return (
    ...
    >
      <Tooltip content={tooltip?.buttonTooltipContent} side={tooltip?.side}>
        <button
          onClick={onClick}
          className={`${
            isActive && 'bg-[#d0e6f2] group-hover:bg-[#d0e6f2!important]'
      ...
      <DropdownMenu modal={false}>
        <Tooltip content={tooltip?.dropdownTooltipContent} side={tooltip?.side}>
        ...
        <DropdownMenuContent
          ...
          className={`${dropdown?.align === 'start' && 'ml-[-2.5rem]'}`}
          forceMount
        >
